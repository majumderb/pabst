...
DISCRIMINATOR_MODELS_PARAMS = {
    "dnli": {
        ...
        "class_size": 2,
        "embed_size": 768,
        "class_vocab": {"positive": 1, "negative": 1},
        "default_class": 0,
...
def perturb_past(
    ...
    for i in range(num_iterations):
        ...
        if loss_type == PPLM_DISCRIM or loss_type == PPLM_BOW_DISCRIM:
            ...
            concat_for_dnli = torch.cat((persona_last_hidden, (new_accumulated_hidden /
                                    (curr_length + 1 + horizon_length))), dim=1) 

            prediction = classifier(concat_for_dnli)
        ...
        # adherence soft constraint
        min_edit_loss = torch.nn.CrossEntropyLoss()(
            perturbed_past[0].view(-1, perturbed_past[0].size(-1)),
            o2.view(-1))
        
        alpha = 1.0
        loss += alpha * min_edit_loss
        loss_list.append(min_edit_loss)
...
def generate_text_pplm(
    ...
    _, _, persona_all_hidden = model(persona)
...
def run_pplm_example(
    ...
    tok_story = tokenizer.encode(tokenizer.bos_token + story,
            add_special_tokens=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help="Retrieved story"
    )

    parser.add_argument(
        "--persona", type=str, default="",
        help="Underlying persona"
    )
